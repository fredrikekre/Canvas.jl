...
(::Type{CO})(dict) where {CO<:CanvasObject} = json2canvas(CO, dict)
(::Type{CO})(; kwargs...) where {CO<:CanvasObject} = json2canvas(CO, kwargs)
...
function json2canvas(::Type{CO}, data::AbstractDict{T}) where {CO<:CanvasObject,T}
    ...
    CO((begin
            ...
            if v === nothing
                ...
                S = adjust_fieldtype(fieldtype(CO, x), CO, x)
                convert′(S, v)
            end
        end
...
adjust_fieldtype(::Type{T}, ::Type, ::Symbol) where T = T
